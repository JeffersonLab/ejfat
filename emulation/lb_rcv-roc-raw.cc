...
const size_t relen        = 0;          // 8 for flags, data_id
...
//        nBytes = recvfrom(lstn_sckt, (void*)in_buff, sizeof(in_buff), 0, (struct sockaddr *)&src_addr, &addr_size);
        ...
        if(nBytes == -1)
        {
            ...
            char s[1024];
            sprintf ( s, "rcvd %d bytes ", int(nBytes));
...
//std::cerr<<"nBytes: "<<nBytes<<'\n';
        ...
        rs.write((char*)&(in_buff[in_buff_idx][mdlen]), nBytes-mdlen);
        rs.flush();
...
//            sprintf ( s, "Writing %d bytes: ", int(nBytes-mdlen));
            sprintf ( s, "Capture %d bytes for seq %d in buff idx %d: ", int(nBytes-mdlen), int(-1), int(in_buff_idx));
            rslg.write((char*)s, strlen(s));
            rslg.flush();
#if 0
