...
static void printHelp(char *programName) {
    fprintf(stderr,
            ...
            "        [-fout <base name of events written out as files>]",
...
static void parseArgs(int argc, char **argv, int* mtu, int *protocol,
                      ...
                      char* host, char *interface, char *filename, char *basename) {
    ...
    *mtu = 0;
    ...
    while ((c = getopt_long_only(argc, argv, "vhp:i:t:d:s:e:f:r:", long_options, 0)) != EOF) {
        ...
        switch (c) {
            ...
            case 16:
                // base filename of events written to files
                if (strlen(optarg) < 1) {
                    fprintf(stderr, "base file name is bad\n\n");
                    printHelp(argv[0]);
                    exit(-1);
                }
...
int main(int argc, char **argv) {
    ...
    bool sendSync = false;
    bool  writeEventsToFiles = false;

    char syncBuf[28];
    char host[INPUT_LENGTH_MAX], interface[16], filename[256], basename[256];
    ...
    parseArgs(argc, argv, &mtu, &protocol, &entropy, &version, &dataId, &port, &tick,
              ...
              &useIPv6, &bufDelay, &sendSync, host, interface, filename, basename);
