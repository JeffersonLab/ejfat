...
#define EJFAT_DAOS_CONT_LABEL "kv_async"
...
int main(int argc, char **argv) {
    ...
    KVClientAsync kv_client(
        EJFAT_DAOS_POOL_LABEL, daosContName);
    kv_client.setCapSize(MiB);
    std::cout << "DAOS pool usage: " << 100.0 * kv_client.getPoolUsage() << "%\n" << std::endl;
    ...
    if (useIPv6) {
        ...
        // Set & read back UDP receive buffer size
        socklen_t size = sizeof(int);
        int rc = setsockopt(udpSocket, SOL_SOCKET, SO_RCVBUF, &recvBufSize, sizeof(recvBufSize));
        if (rc) {
            std::cerr << "1st setsockopt err, rc: " << rc << std::endl;
        }
        recvBufSize = 0;
        getsockopt(udpSocket, SOL_SOCKET, SO_RCVBUF, &recvBufSize, &size);
        ...
        int optval = 1;
        rc = setsockopt(udpSocket, SOL_SOCKET, SO_REUSEPORT, &optval, sizeof(optval));
        if (rc) {
            std::cerr << "2nd setsockopt err, rc: " << rc << std::endl;
        }
        // Configure settings in address struct
    ...
    while (true) {
        ...
        kv_client.flush(totalEvents, nBytes, dataBuf);
