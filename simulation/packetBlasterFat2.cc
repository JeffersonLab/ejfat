...
int main(int argc, char **argv) {
    ...
    uint64_t tick = 0;
    int cores[10];
    int mtu, version = 2, protocol = 1, entropy = 0, streams = 1;
    int rtPriority = 0;
    uint16_t dataId = 1;
    ...
    parseArgs(argc, argv, &mtu, &protocol, &entropy, &version, &dataId, &port, &tick,
              ...
              &delayPrescale, &tickPrescale, cores, &streams, &debug, &sendnocp,
    ...
    bool setRate = setByteRate || setBufRate;
...
//        elapsed, (microSecItShouldTake - elapsed), lastExcessTime, buffersAtOnce*1000000/elapsed);
        ...
        for (int i=0; i < streams; i++) {
            ...
            index = i + startingStream % streams;

            if (sendnocp) {
                err = sendPacketizedBufferFast(buf, bufSize,
                                               maxUdpPayload, clientSockets[index],
                ...
                err = sendPacketizedBufferSend(buf, bufSize, maxUdpPayload, clientSockets[index],
                                               tick, protocol, entropy, version, dataId, &offset,
                                               packetDelay, delayPrescale, &delayCounter,
                                               firstBuffer, lastBuffer, debug, &packetsSent);
            }
