...
//    char     ll,bb;
    int      ll, bb, ver;
    uint64_t tick;
    uint32_t version, protocol, entropy, offset, length = 0;
    uint16_t srcId;
    ...
    while (true) {
        ...
        // Read UDP packet
        size_t bytesRead = recvfrom(udpSocket, pkt, 9100, 0, nullptr, nullptr);

        // Let's look at the metadata
        // First 2 bytes of LB header are 'L' and 'B' and is exactly 20 bytes in length
        hdrStart = pkt;

  //      if (pkt[0] == 'L' && pkt[1] == 'B') {
            readLbHeader(hdrStart, &ll, &bb, &version, &protocol, &entropy, &tick);
            fprintf(stderr, "LB Hdr: (ll %d) (bb %d) (ver %u) (pro %u) (ent %u) (tik %" PRIu64 ")\n",
                    ll, bb, version, protocol, entropy, tick);
            hdrStart = pkt + 16;
   //     }

        // There should always be an RE header
        parseReHeader(hdrStart, &ver, &srcId, &offset, &length, &tick);
        fprintf(stderr, "RE Hdr: (ver %u) (src %hu) (off %u) (len %u) (tik %" PRIu64 ")\n\n",
                ver, srcId, offset, length, tick);
    }
