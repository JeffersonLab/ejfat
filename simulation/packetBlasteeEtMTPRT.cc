 ...
 *
 * Added Prometheus support making this class an exporter of metrics
 */
...
static void *pidThread(void *arg) {
  ...
  auto& absTimeP = ejfat_be.Add({{"absTime", "value"}});
    ...
    while (true) {
        ...
        // Exporting GRPC metrics to Prometheus
...
static std::thread crow_server_thread(run_crow_server);
