...
//    }
    ...
    serverConsumer::~serverConsumer() {
        ...
        // Unregister this client with the grpc server via simple server
        deregisterWithSimpleServer();
    }
     ...
     */
    serverConsumer::serverConsumer(uint16_t dataPort,
     ...
     * @throws EjfatException if no information about the LB/CP is available or could be parsed.
     */
    serverConsumer::serverConsumer(const std::string &serverAddr, const std::string &dataAddr,
                                   uint16_t serverPort, uint16_t dataPort,
                                   const std::vector<int> &ids,
...

    {
        ...
        ipv6ServerAddr = isIPv6(serverAddr);
    ...
    bool serverConsumer::registerWithSimpleServer() {
        ...
        ssize_t err = 0;
        if (ipv6ServerAddr) {
            err = (int) sendto(serverSocket, buffer, len, 0, (sockaddr * ) & serverAddrStruct6, sizeof(struct sockaddr_in6));
        }
        else {
            err = (int) sendto(serverSocket, buffer, len, 0,  (sockaddr * ) & serverAddrStruct, sizeof(struct sockaddr_in));
        }

        if (err < 0) {
            perror("registerWithSimpleServer");
            return false;
        }
        return true;
    }
    ...
    bool serverConsumer::deregisterWithSimpleServer() {
        ...
        if (err < 0) {
            perror("deregisterWithSimpleServer");
            return false;
        }
        return true;
    }
     ...
     * @param isReady true if ready for more data, else false.
     *
    ...
    /**
     * Method to create a UDP socket for sending messages to a simple server.
     * The server, in turn, talks to the CP with gRPC.
     */
    void serverConsumer::createServerSocket() {
        // Socket for sending messages to simple server
        if (ipv6ServerAddr) {
            ...
            memset(&serverAddrStruct6, 0, sizeof(serverAddrStruct6));
            serverAddrStruct6.sin6_family = AF_INET6;
            serverAddrStruct6.sin6_port = htons(serverPort);
            inet_pton(AF_INET6, serverAddr.c_str(), &serverAddrStruct6.sin6_addr);

            if (connectSocket) {
                int err = connect(serverSocket, (const sockaddr *) &serverAddrStruct6, sizeof(struct sockaddr_in6));
                if (err < 0) {
                    close(serverSocket);
            ...
            memset(&serverAddrStruct, 0, sizeof(serverAddrStruct));
            serverAddrStruct.sin_family = AF_INET;
            serverAddrStruct.sin_port = htons(serverPort);
            serverAddrStruct.sin_addr.s_addr = inet_addr(serverAddr.c_str());
            memset(serverAddrStruct.sin_zero, '\0', sizeof serverAddrStruct.sin_zero);

            if (connectSocket) {
                fprintf(stderr, "Connecting server socket to host %s, port %hu\n", serverAddr.c_str(), serverPort);
                int err = connect(serverSocket, (const sockaddr *) &serverAddrStruct, sizeof(struct sockaddr_in));
                if (err < 0) {
                    close(serverSocket);
                    if (debug) perror("Error connecting UDP server socket:");
                    throw EjfatException("error connecting UDP server socket");
     ...
     */
    void serverConsumer::startupStatisticsThread() {
        // Only want one of these threads running
        if (statThdStarted) return;

        std::thread t(&serverConsumer::statisticsThreadFunc, this);
    ...
    void serverConsumer::statisticsThreadFunc() {
...
//                    printf("%d Latency:  %3.4g nanosec,    %3.4g Avg\n", ids[i], latencyInstAvg, latencyTotalAvg);
     ...
     */
    void serverConsumer::serverThreadFunc() {
...
//std::cerr << "sampleTime = " << adjustedSampleTime << ", totalT = " << totalTime << std::endl;
    ...
    void serverConsumer::startupServerThread() {
        if (serverThdStarted) return;

        std::thread t(&serverConsumer::serverThreadFunc, this);
        serverThread = std::move(t);
        serverThdStarted = true;
    }
     ...
     */
    void serverConsumer::recvThreadFunc(recvThdArg *arg) {
     ...
     */
    void serverConsumer::startupRecvThreads() {
        ...
        for (size_t i = 0; i < ids.size(); ++i) {
            ...
            // Start the thread
            std::thread t(&serverConsumer::recvThreadFunc, this, targ);
     ...
     */
    bool serverConsumer::getEvent(char **event, size_t *bytes, uint64_t* eventNum, uint16_t *srcId) {
